...
export async function dayWithConnection(conn: PoolConnection, date: string, model?: string) {
    ...
    const [dbOrders, ] : [any, any] = await conn.query<mysql.QueryResult>(`
        ...
        FROM broker.order 
            ...
            on broker.order.date = simulations.date 
                and broker.order.minute=simulations.minute
                and broker.order.order=simulations.order
                and broker.order.model_name=simulations.model_name
