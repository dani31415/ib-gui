...
export async function dayWithConnection(conn: PoolConnection, date: string) {
    ...
    const [dbOrders, ] : [any, any] = await conn.query<mysql.QueryResult>(`
        SELECT symbol_src_name as name, count(*) as count, avg(sell_order_price/buy_order_price) as gains1, avg(gains2) as gains2 
        FROM broker.order 
            ...
            order by gains1 desc, gains2 desc, symbol_src_name
    `, [date]);
